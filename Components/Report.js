...
class NewReport extends Component {
  ...
  submitReport = async () => {
    ...
    if (errors.length === 0) {
      ...
      let received = await DatabaseService.sendReport(TypeOfViolation, TellUsMore, id); // send report to backend, wait for response
      if (received) {
  ...
  render() {
    return (
      <View style={styles.content}>
        {/* new report form */}
        <Form type={ReportSchema} options={ReportOptions} ref={c => (this.form = c)} />
        <View>
          <Button bordered info rounded onPress={() => this.submitReport()}>
